...
# import methods for the BusKill class that have been separated into distinct files for readability	
import packages.buskill._buskill_upgrade

class BusKill( _buskill_upgrade.Mixin ):
	...
	def __init__(self):
		...
		self.BUSKILL_VERSION = BUSKILL_VERSION
		
		self.RELEASE_KEY_FINGERPRINT = 'E0AFFF57DC00FBE0563587614AE21E1936CE786A'
		self.RELEASE_KEY_SUB_FINGERPRINT = '798DC1101F3DEC428ADE124D68B8BCB0C5023905'
		...
		self.RUNTIME_DIR = sys.path[0]
		# APP_DIR is the dir in which our buskill executable lives
		self.APP_DIR = None
		self.APPS_DIR = None
		self.DATA_DIR = None
		self.CACHE_DIR = None
		...
		# documentation links
		if self.BUSKILL_VERSION['VERSION'] == '':
			ver = 'stable'
		else:
			ver = self.BUSKILL_VERSION['VERSION']
		...
		#               APP_DIR)
		# 4. RUNTIME_DIR = the directory that contains the 'src' dir, where the KEYS
		#                  file (and others) live
